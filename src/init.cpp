 ...
 */
static const char* BITCOIN_PID_FILENAME = "osmanli_altin_coind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("osmanli_altin_coin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/osmanlialtincoin-project/osmanlialtincoin>";
    const std::string URL_WEBSITE = "<https://osmalialtincoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("osmanli_altin_coin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if osmanli_altin_coin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if osmanli_altin_coin is started while in a temporary directory.\n",
