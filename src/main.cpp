...
#if defined(NDEBUG)
# error "Osmanlialtincoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x339ee2ea4f02891e13050c780016c899f9884bf95f708bfc76138cc2e5441105");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Osmanlialtincoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Osmanlialtincoin Signed Message:\n";
...
{
    // Osmanlialtincoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Osmanlialtincoin: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Osmanlialtincoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Osmanlialtincoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Osmanlialtincoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Osmanlialtincoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x179e685d194e7077ff68106458b869a314ea813f3623c2033dc5e4ac3097b17b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12/28/2021 ; Mansa Musa sizleri seviyor";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04984da74a7b0e17a293f3a21074e1ada7475d21b6d8691e4810375a4505b3151390a88881d654f7248a4d7b597367c113720db02fabebe4c04642201db0be717f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1640693260;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085889145;

        if (fTestNet)
        {
            block.nTime    = 1640693223;
            block.nNonce   = 385523513;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x041dcf81ac818d3cfbf8cc1e0604a59132b0a2b4821ce748be563110e2f5c85e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc4, 0xbb, 0xd6 }; // Osmanlialtincoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OsmanlialtincoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OsmanlialtincoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OsmanlialtincoinMiner(CWallet *pwallet)
{
    printf("OsmanlialtincoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("osmanlialtincoin-miner");
    ...
    try { loop {
        ...
        printf("Running OsmanlialtincoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OsmanlialtincoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OsmanlialtincoinMiner, pwallet));
}
