# PSBT Howto for Osmanli_altin_coin Core

Since Osmanli_altin_coin Core 0.17, an RPC interface exists for Partially Signed Osmanli_altin_coin
...
PSBT is an interchange format for Osmanli_altin_coin transactions that are not fully signed
...
## PSBT in Osmanli_altin_coin Core
...
#### Multisig with multiple Osmanli_altin_coin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Osmanli_altin_coin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `osmanli_altin_coin-cli` is used.
