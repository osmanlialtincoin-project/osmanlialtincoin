...
- [Development guidelines](#development-guidelines)
    - [General Osmanli_altin_coin Core](#general-osmanli_altin_coin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Osmanli_altin_coin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play osmanli_altin_coins" on the test network, if you
...
Osmanli_altin_coin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/osmanli_altin_coin-project/osmanli_altin_coin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_osmanli_altin_coin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_osmanli_altin_coin --log_level=test_suite
$ valgrind -v --leak-check=full src/osmanli_altin_coind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_osmanli_altin_coin.coverage/index.html`.
```
...
To profile a running osmanli_altin_coind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep osmanli_altin_coind` -- sleep 60
```
...
Osmanli_altin_coin Core can be compiled with various "sanitizers" enabled, which add
...
- ThreadSocketHandler : Sends/Receives data from peers on port 8402.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 8401 for connections and services them.
...
However, in open source software such as Osmanli_altin_coin Core, where everyone uses
...
pay attention to for reviewers of Osmanli_altin_coin Core code.

General Osmanli_altin_coin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Osmanli_altin_coin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Osmanli_altin_coin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Osmanli_altin_coin Core so that they can be integrated
...
file descriptors it will cause problems with Osmanli_altin_coin's `select()` loop, because
...
```bash
$ lsof -p $(pidof osmanli_altin_coind) |\
...
would be to revert the upstream fix before applying the updates to Osmanli_altin_coin's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `osmanli_altin_coin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `osmanli_altin_coin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Osmanli_altin_coin Core, the wallet was always
